...
//  opts.name           - string, the nickname of the bot
//  opts.group_id       - string, the group the bot can post to
//  opts.avatar_url     - string, ImageService url to avatar
//  opts.callback_url   - string, a callback for new messages
api.Bots.update = function(at, bot_id, opts, callback){
  var payload = {
    "bot[bot_id]": bot_id
  }
  if (opts && opts.avatar_url)
    payload["bot[name]"] = opts.name;
  if (opts && opts.avatar_url)
    payload["bot[group_id]"] = opts.group_id;
  if (opts && opts.avatar_url)
    payload["bot[avatar_url]"] = opts.avatar_url;
  if (opts && opts.callback_url)
    payload["bot[callback_url]"] = opts.callback_url;

  postRequest(at, "/bots/" + bot_id, {form:payload, responseCode:202, doParse:true}, callback);
}
